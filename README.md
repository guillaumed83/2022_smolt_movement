...
Receiver observations are **coauthor-supplied data used as provided**. No additional false-detection screening is known and no new filter was applied. Event curation preserves the exact observations selected by the executed analysis and all 540 modeled binary detection-history cells. The `_edited.csv` source suffix does not establish a cleaning procedure.

Tide, release and light clock labels follow **Quebec local time**, confirmed with the data provider. The public zone is `America/Toronto`. Its UTC-04 offsets and instants were verified equivalent to the legacy zone over the full 2022 analysis envelope; no one-hour shift was made. Island-network passage ends at the **last** D10/D11 detection. Marine intervals start at the last HoT detection.
